...
class CheckInController {
    ...
    async createCheckIn(req, res) {
        try {
            ...
            if (qrCodeId) {
                ...
                let qrCode = await prisma_1.prisma.qRCode.findUnique({
                    where: { id: qrCodeId },
                });
            ...
            if (type === 'CHECK_OUT') {
                ...
                const lastCheckIn = await prisma_1.prisma.checkInOut.findFirst({
                    where: {
            ...
            const checkIn = await prisma_1.prisma.checkInOut.create({
                data: {
                    employeeId: employee.id,
            ...
            const attendanceRecord = await prisma_1.prisma.attendanceRecord.findUnique({
                where: {
                    employeeId_date: {
                        employeeId: employee.id,
                        date: today
    ...
    async getCheckIns(req, res) {
        try {
            ...
            const checkIns = await prisma_1.prisma.checkInOut.findMany({
    ...
    async getTodayCheckIn(req, res) {
        try {
            ...
            const todayCheckIns = await prisma_1.prisma.checkInOut.findMany({
                where: {
                    employeeId: employee.id,
                    timestamp: {
                        gte: today,
    ...
    async submitAttendanceRequest(req, res) {
        try {
            const userId = req.userId;
            ...
            if (attendanceRecord) {
                ...
                if (parsedCheckIn !== undefined)
                    updateData.checkIn = parsedCheckIn;
                if (parsedCheckOut !== undefined)
                    updateData.checkOut = parsedCheckOut;
                ...
                if (parsedCheckIn !== undefined)
                    createData.checkIn = parsedCheckIn;
                if (parsedCheckOut !== undefined)
                    createData.checkOut = parsedCheckOut;
