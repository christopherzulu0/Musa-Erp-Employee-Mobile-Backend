...
class AdvanceController {
    ...
    async createAdvanceRequest(req, res) {
        try {
            ...
            // Create advance request
            const advanceRequest = await prisma_1.prisma.employeeAdvance.create({
                data: {
    ...
    async getAdvanceRequests(req, res) {
        try {
            const userId = req.userId;
            if (!userId) {
                return res.status(401).json({ success: false, error: 'User not authenticated' });
            ...
            const advanceRequests = await prisma_1.prisma.employeeAdvance.findMany({
                where: {
                    employeeId: employee.id
                },
                orderBy: {
    ...
    async getAdvanceRequestById(req, res) {
        try {
            const userId = req.userId;
            const { id } = req.params;
            if (!userId) {
            ...
            // Get advance request
            const advanceRequest = await prisma_1.prisma.employeeAdvance.findFirst({
                where: {
                    id,
                    employeeId: employee.id
                },
...
//# sourceMappingURL=advanceController.js.map