...
export class PerformanceController {
  ...
  async getPerformanceMetrics(req: Request, res: Response) {
    try {
      ...
      const energyPointsResult = await prisma.energyPoint.aggregate({
        where: { employeeId: employee.id },
        _sum: { points: true }
      });
  ...
  // Get energy points
  async getEnergyPoints(req: Request, res: Response) {
    try {
      const userId = (req as any).userId;
      if (!userId) {
        return res.status(401).json({ success: false, error: 'User not authenticated' });
      ...
      const energyPoints = await prisma.energyPoint.findMany({
        where: { employeeId: employee.id },
      ...
      const awardedByEmployees = await prisma.employee.findMany({
        where: { 
      ...
      const employeeMap = new Map(
        awardedByEmployees.map(emp => [emp.userId, `${emp.firstName} ${emp.lastName}`])
      );
