...
export class LeaveController {
  ...
  async getLeaveBalances(req: Request, res: Response) {
    try {
      ...
      const currentPeriod = await prisma.leavePeriod.findFirst({
        where: { isActive: true },
  ...
  async createLeaveApplication(req: Request, res: Response) {
    try {
      const userId = (req as any).userId;
      if (!userId) {
        return res.status(401).json({ success: false, error: 'User not authenticated' });
  ...
  // Get upcoming leaves
  async getUpcomingLeaves(req: Request, res: Response) {
    try {
      const userId = (req as any).userId;
      if (!userId) {
        return res.status(401).json({ success: false, error: 'User not authenticated' });
      ...
      const upcomingApplications = await prisma.leaveApplication.findMany({
        where: {
          employeeId: employee.id,
          status: 'APPROVED',
          startDate: { gte: today }
  ...
  async createCompensatoryLeaveRequest(req: Request, res: Response) {
    try {
      const userId = (req as any).userId;
      if (!userId) {
        return res.status(401).json({ success: false, error: 'User not authenticated' });
      ...
      const compensatoryRequest = await prisma.compensatoryLeaveRequest.create({
        data: {
          employeeId: employee.id,
  ...
  async getCompensatoryLeaveRequests(req: Request, res: Response) {
    try {
      const userId = (req as any).userId;
      if (!userId) {
        return res.status(401).json({ success: false, error: 'User not authenticated' });
      ...
      const compensatoryRequests = await prisma.compensatoryLeaveRequest.findMany({
        where: { employeeId: employee.id },
        orderBy: { appliedAt: 'desc' },
        take: 10
      });
